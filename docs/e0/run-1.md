# E0 run 1 — friction notes from writing Pong against the API document
...
The game was `crates/jidousha/examples/pong/`, since replaced by run 2's; run
1's is in `main`'s history. It compiled on the first attempt and `--verify` was
green. That is not the interesting part of this file.

Run 2 is in `run-2.md`, and the two are **deliberately separate files**. Run 2
was written into the same file as this one and had to read it first, so it knew
the timestep was 1/60 and that `Key::ArrowUp` existed before it opened the API
document. One file per run is what makes the next measurement clean.
