...
impl Art {
    ...
    /// The marker icon for a location's role.
    ///
    /// The town takes the heart — home is the one signifier the quest icons
    /// do not cover, and the heart is the closest existing role (the fork's
    /// UI.md records the reassignment).
    pub fn for_icon(icon: IconRole) -> Art {
        match icon {
            IconRole::Town => Art::Heart,
            IconRole::Cave => Art::QuestCave,
            IconRole::Crypt => Art::QuestCrypt,
            IconRole::Tower => Art::QuestTower,
            IconRole::Vault => Art::QuestVault,
        }
    }
}
