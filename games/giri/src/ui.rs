...
impl IconRun {
    ...
    /// The same, on an overlay.
    pub fn over(at: Vec2, art: Art, scale: f32) -> Self {
        Self {
            layer: theme::layers::OVERLAY_TEXT,
            ..Self::new(at, art, scale)
        }
    }
...
impl Panel {
    ...
    /// Every string in this panel, for the printable check.
    pub fn strings(&self) -> impl Iterator<Item = &str> {
        self.runs.iter().map(|run| run.text.as_str())
    }
...
/// A filled, bordered panel — the shape every card and column on the board is.
pub fn card(ctx: &mut DrawCtx, rect: Rect, background: Color, edge: Color, layer: i16) {
    fill(ctx, rect, background, layer);
    border(ctx, rect, edge, 2.0, layer + 1);
}
...
/// A ghost button — the secondary shape, for RELEASE and LOG.
pub fn ghost_button(ctx: &mut DrawCtx, rect: Rect, layer: i16) {
    fill(
        ctx,
        Rect::from_min_size(rect.min + Vec2::new(0.0, 3.0), rect.size()),
        theme::GROUND,
    ...
    fill(ctx, rect, theme::GHOST, layer + 1);
    border(ctx, rect, theme::BORDER, 2.0, layer + 1);
}
...
/// Draw a panel's contents: every icon, then every row.
pub fn draw(ctx: &mut DrawCtx, panel: &Panel) {
