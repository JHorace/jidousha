...
impl Checks {
    ...
    /// Everything that failed, one line each - what a mutation round reads to
    /// find out *which* check noticed.
    pub fn first_failure(&self) -> Option<String> {
        self.problems
            .first()
            .map(|(what, specifics)| format!("{what}: {specifics}"))
    }
...
fn complaint(what: &str, specifics: &str) -> String {
    message(
        ...
        "run `cargo run -p giri` and play the beat, then compare with the assertion above",
    )
}
