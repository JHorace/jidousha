...
*Landed 2026-08-23:* the placeholder library, generated by
`art/make_placeholders.py` from the grids in `art/sprite_defs.py` — four
portraits, four dungeon icons, five stat and event icons. The slots are final
and the pictures are not: the owner's library replaces the files by name and no
code changes (UI.md §9). `art/import_pack.py` is the door that library comes in
through.
