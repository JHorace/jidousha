...
reviewing the captures (DECIDED, 2026-08-23)**: this is giri's art, the licence
question is closed (original work of this repository), and a change to how the
game looks is a change to a grid. `art/import_pack.py` stays as the door a
curated library would come in through — the role naming means one would replace
files by name and change no code (UI.md §9) — but nothing is waiting on it.
