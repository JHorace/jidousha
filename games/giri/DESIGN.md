...
**Assets: giri v1 has none.** The whole game is tier-1 of the asset ladder —
tinted quads, circles, lines, and built-in text; a character "portrait" is a
colored quad with an initial on it. No PNGs, no `Assets` store, no
placeholder policy in play. If later versions want real sprites, they are
generated by a committed script (original art, deterministic, reviewable) or
owner-provided — **never downloaded from third-party sources by an agent**
(provenance/licensing; owner approval + recorded license is the only
exception).

Screens: roster/sheet inspection (invariant 2: everything inspectable);
party assembly with **live willingness preview** — refusals shown with their
