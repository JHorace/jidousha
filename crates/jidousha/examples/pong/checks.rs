...
pub(crate) struct Checks {
    /// What failed, and the numbers it judged.
    problems: Vec<(String, String)>,
}

impl Checks {
    /// Record a check, keeping its complaint if it did not hold.
...
/// fault worth reporting beside the others, while a ball that is *gone* leaves
...
fn complaint(what: &str, specifics: &str) -> String {
    message(
        ...
        "run `cargo run -p jidousha --example pong` and watch it, then compare \
...
/// position satisfies every plain `<=` and would pass this verification.
...
/// The sizes of every quad covering a point, for a message that has to say
/// what it found rather than only what it wanted.
